...
% Check each variable and check if it is correct or not.
    ...
    %%% 
        ...
        location_data_to = "D:\UGent\Burgie\Jaar 4\Thesis\Datasets\Dataset2\Results (Sensor)\"; %full path to the directory where the new data needs to be stored. 
        location_data_information = "D:\UGent\Burgie\Jaar 4\Thesis\Datasets\Dataset2\Results (Sensor)\"; %full path to the directory where the dataset information needs to be stored
        location_data_statistics = "D:\UGent\Burgie\Jaar 4\Thesis\Datasets\Dataset2\Results (Sensor)\"; %full path to the directory where the statistical .csv file needs to be stored.
    ...
    %%%
        ...
        pow_results_map_name = 'Power-Results-Test';
        fc_results_map_name = 'Functional-Connectivity-Results-Test';
        dynfc_resuls_map_name = '';
        dyncausal_results_map_name = '';
        graph_results_map_name = '';
        microstates_results_map_name = 'Microstates-Results-Test';
...
if(run_analysis_microstates == 1)
    ...
    % SUBSTEP 2: MAIN FOR LOOP
    %%%
        all_participants_maps_at_gfp_peaks = [];
        all_participants_region_timeseries = [];
        ...
        for participant_i = 1:1:dataset_size
            ...
            [current_participant_region_timeseries, current_participant_region_names] = Extract_Sensor_Time_Series_And_Names(current_participant_table,...
                                                                                                                             electrode_layout_information);
            %all_participants_region_timeseries = [all_participants_region_timeseries current_participant_region_timeseries];
            ...
            %[current_participant_microstates, current_participant_microstate_labels, current_participant_gev] = ...
            %        Microstates(current_participant_region_timeseries, sample_frequency, epoch_length, ...
            %        4, "k-means", {100});

            %disp(current_participant_gev)

            %Plot_Sensor_Topography(centroids(1,:), electrode_locations);
            %Plot_Sensor_Topography(centroids(2,:), electrode_locations);
            %Plot_Sensor_Topography(centroids(3,:), electrode_locations);
            %Plot_Sensor_Topography(centroids(4,:), electrode_locations);

%             %save the results in the previously defined map
%             Save_Results_To_Directory(current_participant_values,current_participant_name,FC_Results_map);
        end
        %Clustering(all_participants_gfp_timeseries, 4, 100, 5);
        microstates = Cluster_Microstates(all_participants_microstates, 4, "modified k-means", {1, 100});
